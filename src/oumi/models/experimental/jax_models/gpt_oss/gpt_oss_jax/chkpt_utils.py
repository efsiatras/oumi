...
def quantize_model(ckpt_path: Path, quant_ckpt_path: Path) -> None:
    ...
    cfg = dataclasses.replace(
        cfg, mesh=mesh, quant_moe=True, quant_attn=False
    )  # do not quantize attention

    print("Loading weights...")
    weights = gpt_jax.load_pytree(
        ckpt_path,
        gpt_jax.Weights.shardings(
    ...
    quant_layers = [
        gpt_jax.Layer.quantize(layer, cfg)
...
def dequantize_mxfp4(
    blocks_2x_e2m1: torch.Tensor, scales_e8m0: torch.Tensor, dtype=torch.bfloat16
):
    scales_e8m0 = torch.as_tensor(scales_e8m0, dtype=torch.float32)
    scales = (2.0 ** (scales_e8m0 - 127))[..., None, None]
    x = (
        torch.stack([e2m1_to_fp(blocks_2x_e2m1), e2m1_to_fp(blocks_2x_e2m1 >> 4)], -1)
        * scales
    )
    ...
    if len(mxfp4_keys) > 0:
        ...
        for key in tqdm(mxfp4_keys):
            ...
            weight = dequantize_mxfp4(
                torch_params[root + "_blocks"], torch_params[root + "_scales"]
            ).contiguous()
