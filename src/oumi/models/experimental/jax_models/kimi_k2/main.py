#!/usr/bin/env python3
"""Minimal Kimi K2 inference using JAX
Based on jax-llm-examples/kimi_k2/main.py
"""
...
if __name__ == "__main__":
    ...
    ckpt_path = Path("~/models/jax/moonshot-ai--Kimi-K2-Chat").expanduser()

    if not ckpt_path.exists():
        print("❌ Model not found. Please:")
        print("1. Download model:")
        print(
            "   python scripts/download_model.py --model-id moonshot-ai/Kimi-K2-Chat --dest-root-path ~/models/hf/"
        )
        print("2. Convert to JAX:")
        print(
            "   python scripts/convert_weights.py --source-path ~/models/hf/moonshot-ai--Kimi-K2-Chat --dest-path ~/models/jax/moonshot-ai--Kimi-K2-Chat"
        )
        exit(1)
    ...
    # Load tokenizer
    tokenizer = k2jax.load_tokenizer(
    ...
    cfg = k2jax.hf_to_jax_config(json.loads((ckpt_path / "config.json").read_text()))
    ...
    # Load weights
    weights = k2jax.load_pytree(ckpt_path, k2jax.Weights.shardings(cfg))
    ...
    with set_mesh(cfg.mesh):
        zero_cache = k2jax.KVCache.init(
            random.key(1), cfg, input.shape[0], cfg.max_seq_len
        )
        next_tokens, logits, cache = k2jax.prefill(input, weights, zero_cache, cfg)
