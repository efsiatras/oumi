...
def attention_block(
    ...
    with jax.named_scope("rope"):
        q, k = (
            rms_norm(q, layer.q_gamma, cfg.norm_eps),
            rms_norm(k, layer.k_gamma, cfg.norm_eps),
        )
...
def _route_tokens_to_moe_experts(
    ...
    topk_weights, topk_idx = jax.lax.top_k(
        jax.nn.softmax(scores, axis=-1), cfg.moe_experts_per_tok
    )
...
def forward_layer(
    ...
    with jax.named_scope("ffn"):
        ff_out = (moe_block if is_type(layer.ffw, MoELayer) else mlp_block)(
            ff_in, layer.ffw, cfg
        )
    with jax.named_scope("residual"):
